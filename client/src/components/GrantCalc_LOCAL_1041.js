...
const FormG = () => {
    ...
    const handleSFTA = () => event => {
        SFTAsetState({...SFTAstate, spouseFirstTimeApplicant: event.target.value});
    }

    const handleIncome = () => event => {
        incomesetState({...incomestate, incomeLevel: parseInt(event.target.value)});
    }

    const handleEmployed = () => event => {
        employmentsetState({...employmentstate, employmentStatus: event.target.value});
    }
    ...
    const handleSubmit = () => event => {
      ...
      >
        <DialogTitle id="alert-dialog-slide-title">{"Your Available Grant Amount"}</DialogTitle>
        <DialogContent>
